...
export async function analizarContenido(
  ...
  const response = await fetch(`${BASE_URL}/predecir`, {
    method: "POST",
...
/**
 * Comprueba si el microservicio FastAPI está disponible.
 * FastAPI expone `/docs` y `/openapi.json` aunque no tenga un endpoint health.
 */
export async function chequearSalud(path = "/docs"): Promise<boolean> {
  const rutas = Array.from(new Set([path, "/docs", "/openapi.json"]));
